...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) // runs MongoClient's connect method, passing connectionStr as first arg and setting the unifiedtopology flag to true in 2nd arg
    .then(client => { // awaits mongoclient's connect method, then executes a callback function with the client obj passed to it
...
//READ
app.get('/',async (request, response)=>{ // sets a GET method on the '/' route. 2nd argument is an asynchronous callback function for what to do on that route
...
// CREATE
app.post('/addTodo', (request, response) => { //runs a post request that can add a new to-do as a document in the collection
    db.collection('todos').insertOne({thing: request.body.todoItem, completed: false}) //accesses the specified database and collection and uses MongoDB's insertOne method to insert a new document into the todo's collection
    .then(result => { //awaits the addition to the collection
...
//UPDATE
app.put('/markComplete', (request, response) => { //runs a put request which will update to-dos in the collection
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //accesses the specified collection and using MongoDB's updateOne method it will change the properties of the todolist accordingly
        $set: {
...
app.put('/markUnComplete', (request, response) => {
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //accesses the specified database and collection and uses MongoDB's updateOne method to update a document in the todo's collection
        $set: {
...
//DELETE
app.delete('/deleteItem', (request, response) => { // runs a delete request to delete a todo item
    db.collection('todos').deleteOne({thing: request.body.itemFromJS}) //accesses the specified database and collection and uses MongoDB's deleteOne method to delete a new document from the todo's collection
    .then(result => {
...
app.listen(process.env.PORT || PORT, ()=>{ // listens for a specified port to load the server
